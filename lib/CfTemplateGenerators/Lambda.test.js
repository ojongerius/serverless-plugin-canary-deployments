...
describe('Lambda', () => {
  ...
  describe('.replaceEventInvokeConfigQualifierWithAlias', () => {
    ...
    it('replaces the destination event invoke config \'s qualifier for an alias', () => {
      const functionAlias = 'TheFunctionAlias'
      const expected = _.set('Properties.Qualifier', 'TheFunctionAlias', eventInvokeConfig)
      const actual = Lambda.replaceEventInvokeConfigQualifierWithAlias(eventInvokeConfig, functionAlias)
      expect(actual).to.deep.equal(expected)
    })
  })
})
